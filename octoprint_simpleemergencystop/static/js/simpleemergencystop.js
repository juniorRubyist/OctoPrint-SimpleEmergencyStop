...
$(function() {
    function SimpleemergencystopViewModel(parameters) {
        ...
        self.onAfterBinding = function() {
            ...
            self.settings =
                self.allSettings.settings.plugins.simpleemergencystop;
        };

        self.click = function() {
            if (self.settings.confirmationDialog())
                self.confirmation.modal("show");
            else self.sendCommand();
        };
        ...
        self.visibleTest = function() {
            return (
                self.loginState.isUser() && self.printerState.isOperational()
            );
        };

        self.showText = function() {
            return self.settings.showText();
        };
    }
